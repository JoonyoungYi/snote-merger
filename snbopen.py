...
def addText(canvas, element, styles):
    for run in element.getElementsByTagName("sn:r"):
        if (len(run.getElementsByTagName("sn:t")) > 0):
            ...
            charStyle = styles["Character" +
                               run.getAttributeNode("sn:rStyle").value]
    ...
    for style in styles.getElementsByTagName("sn:style"):
        if style.getAttributeNode("sn:type").value == "character":
            if len(style.getElementsByTagName("sn:color")) > 0:
                color = style.getElementsByTagName("sn:color")[0].getAttribute(
                    "sn:val")
            else:
                color = "000000"
            if len(style.getElementsByTagName("sn:sz")) > 0:
                size = int(
                    style.getElementsByTagName("sn:sz")[0].getAttribute(
                        "sn:val")) * .5
            else:
                size = 16
            charStyles[style.getAttribute("sn:styleId")] = Style(
                len(style.getElementsByTagName("sn:b")) > 0,
                len(style.getElementsByTagName("sn:i")) > 0,
                len(style.getElementsByTagName("sn:u")) > 0, color, size)
    return charStyles
    ...
    for element in bodyElements:
        if element.nodeName == "sn:page":
            if 'pdfCanvas' in vars():
                ...
                pdfCanvas = canvas.Canvas(
                    pdfname if pdfname else snbname.replace(".snb", ".pdf"))
        ...
        #ussually images
            ...
            if len(images) != 0:
                ...
                print "unknown SNoteObj" + "on page " + str(
                    pdfCanvas.getPageNumber())

        elif element.nodeName == "sn:l":
            addText(pdfCanvas, element, charStyles)

        else:
            print "unknown element type:" + element.nodeName + " on page " + str(
                pdfCanvas.getPageNumber())
...
if len(argv) == 2:
    pdfFileName = gettempdir() + sep + sub('.*' + sep, "", argv[1]).replace(
        ".snb", ".pdf")
